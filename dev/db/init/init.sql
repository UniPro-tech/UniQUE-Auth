...
ALTER TABLE `token_sets`
ADD FOREIGN KEY (`refresh_token_id`) REFERENCES `refresh_tokens` (`id`);
ALTER TABLE `token_sets`
ADD FOREIGN KEY (`id_token_id`) REFERENCES `id_tokens` (`id`);
ALTER TABLE `auths`
ADD FOREIGN KEY (`auth_user_id`) REFERENCES `users` (`id`);
...
ADD FOREIGN KEY (`auth_id`) REFERENCES `auths` (`id`);
ALTER TABLE `oidc_authorizations`
ADD FOREIGN KEY (`consent_id`) REFERENCES `consents` (`id`);
ALTER TABLE `oidc_authorizations`
ADD FOREIGN KEY (`code_id`) REFERENCES `code` (`id`);
ALTER TABLE `token_sets`
ADD FOREIGN KEY (`oidc_authorization_id`) REFERENCES `oidc_authorizations` (`id`);
...
SET src = src DIV 32;
IF i < encoded_len THEN ITERATE ENCODE;
END IF;
LEAVE ENCODE;
END LOOP ENCODE;
RETURN result;
END;
// CREATE FUNCTION gen_ulid () RETURNS CHAR(26) NOT DETERMINISTIC READS SQL DATA BEGIN
...
END;
// DELIMITER;
DELIMITER // CREATE TRIGGER before_insert_users BEFORE
INSERT ON users FOR EACH ROW BEGIN IF NEW.id IS NULL
  OR NEW.id = '' THEN
SET NEW.id = gen_ulid();
END IF;
END;
// DELIMITER;
DELIMITER // CREATE TRIGGER before_insert_apps BEFORE
INSERT ON apps FOR EACH ROW BEGIN IF NEW.id IS NULL
  OR NEW.id = '' THEN
SET NEW.id = gen_ulid();
END IF;
END;
// DELIMITER;
DELIMITER // CREATE TRIGGER before_insert_roles BEFORE
INSERT ON roles FOR EACH ROW BEGIN IF NEW.id IS NULL
  OR NEW.id = '' THEN
SET NEW.id = gen_ulid();
END IF;
END;
// DELIMITER;
DELIMITER // CREATE TRIGGER before_insert_sessions BEFORE
INSERT ON sessions FOR EACH ROW BEGIN IF NEW.id IS NULL
  OR NEW.id = '' THEN
SET NEW.id = gen_ulid();
END IF;
END;
